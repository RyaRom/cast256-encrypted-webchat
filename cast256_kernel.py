...
def substitution_quarter_image(abcd):
    """
    Эта функция выполняет ПОДСТАНОВКУ. Она разбивает 32-битный блок на 4 блока по 8 бит.
    Затем выполняется подстановка этих 4 значений с использованием таблиц подстановки CAST-256.
    Таблицы с 1 по 4 применяются к 8-битным блокам от самого старшего к младшему, то есть,
    слева направо в порядке появления в исходном 32-битном блоке.
    :param abcd: 32-битный блок
    :return: 4 подставленных значения из 4 таблиц подстановки, соответствующие 4 блокам по 8 бит
    """
    # Décomposer le bloc de 32 bits en 4 blocs de 8 bits.
    a, b, c, d = utils.extract_8bit_blocs_from_32(abcd)
    # Application des tables de subtitutions du CAST-256.
    # a -> s_box[1]
    a_sub = s_box_1[a]
    # b -> s_box[2]
    b_sub = s_box_2[b]
    ...
    # Retourner les 4 blocs de 8 bits.
    return a_sub, b_sub, c_sub, d_sub


def function1(d, kri, kmi):
    """
    Функция 1 алгоритма CAST-256. Выполняет следующие операции: сложение по модулю 2^32,
    циклический сдвиг влево, подстановка и XOR — в порядке, указанном в документации.
    :param d: обрабатываемый блок (32 бита)
    :param kri: ключ сдвига (8 бит)
    :param kmi: маскирующий ключ (32 бита)
    :return: результат операций (32 бита)
    """
    data = utils.sum_mod_232(kmi, d)
    data_shifted = utils.shift_left(data, 32, kri)
    a, b, c, d = substitution_quarter_image(data_shifted)
    return utils.sum_mod_232(utils.diff_mod_232(a ^ b, c), d)


def function2(d, kri, kmi):
    """
    Функция 2 алгоритма CAST-256. Выполняет операции: XOR, циклический сдвиг влево,
    подстановка и последующие арифметические операции.
    :param d: обрабатываемый блок (32 бита)
    :param kri: ключ сдвига (8 бит)
    :param kmi: маскирующий ключ (32 бита)
    :return: результат операций (32 бита)
...
def function3(d, kri, kmi):
    """
    Функция 3 алгоритма CAST-256. Выполняет операции: вычитание по модулю 2^32,
    циклический сдвиг влево, подстановка и комбинации арифметических/логических операций.
    :param d: обрабатываемый блок (32 бита)
    :param kri: ключ сдвига (8 бит)
    :param kmi: маскирующий ключ (32 бита)
    :return: результат операций (32 бита)
    """
    data = utils.diff_mod_232(kmi, d)
    data_shifted = utils.shift_left(data, 32, kri)
    a, b, c, d = substitution_quarter_image(data_shifted)
    return utils.diff_mod_232(utils.sum_mod_232(a, b) ^ c, d)


def forward_quad_round(abcd, kr, km):
    """
    Функция прямого раунда (forward_quad_round) алгоритма CAST-256. Разбивает входной 128-битный блок
    на четыре 32-битных блока. Эти блоки обрабатываются функциями f1, f2 и f3 с использованием
    ключей сдвига и маски. Полученные блоки объединяются обратно в 128-битный блок.
    !!! ВАЖНО: СОБЛЮДАЙТЕ ПОРЯДОК ОПЕРАЦИЙ, УКАЗАННЫЙ В ДОКУМЕНТАЦИИ !!!
    :param abcd: входной блок (128 бит)
    :param kr: список из 4 ключей сдвига (по 8 бит)
    :param km: список из 4 масок (по 32 бита)
    :return: результат (128 бит)
    """
    a, b, c, d = utils.extract_32bit_bloc_from_128(abcd)
    c = c ^ function1(d, kr[0], km[0])
    b = b ^ function2(c, kr[1], km[1])
    a = a ^ function3(b, kr[2], km[2])
    d = d ^ function1(a, kr[3], km[3])
    return utils.build_128_bit_bloc_from_32_bit_blocs(a, b, c, d)


def reverse_quad_round(abcd, kr, km):
    """
    Функция обратного раунда (reverse_quad_round) алгоритма CAST-256. Разбивает входной 128-битный блок
    на четыре 32-битных блока. Эти блоки обрабатываются функциями f1, f2 и f3 с использованием
    ключей сдвига и маски в обратном порядке. Полученные блоки объединяются обратно в 128-битный блок.
    !!! ВАЖНО: СОБЛЮДАЙТЕ ПОРЯДОК ОБРАТНЫХ ОПЕРАЦИЙ, УКАЗАННЫЙ В ДОКУМЕНТАЦИИ !!!
    :param abcd: входной блок (128 бит)
    :param kr: список из 4 ключей сдвига (по 8 бит)
    :param km: список из 4 масок (по 32 бита)
    :return: результат (128 бит)
